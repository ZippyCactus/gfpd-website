...
# Smart Manual Deployment v2 - Handles file moves and deletions
...
echo -e "${BLUE}🚀 Smart Manual Deployment v2${NC}"
echo "=================================="
echo ""
...
if [[ ! -f "$DEPLOYMENT_LOG" ]]; then
    ...
    RENAMED_FILES=$(git diff --name-only --diff-filter=R $LAST_COMMIT HEAD | grep -v "^\.git" | grep -v "deploy-" | grep -v "setup-" | grep -v "upload-")
    
    # Combine all changes
    CHANGED_FILES="$ADDED_FILES $MODIFIED_FILES $RENAMED_FILES"
